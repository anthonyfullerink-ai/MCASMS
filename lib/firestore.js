...
/**
 * Increment the minutesUsed counter and deduct from voiceMinutesBalance for the matched subscriber.
 * True whole-number metering: rounds duration to nearest whole minute (e.g. 4.3m -> 4m, 15.6m -> 16m).
 * When balance reaches 0, atomically sets isVoicePaused=true, voiceActive=false, aiSmsActive=false.
 * @param {string} keyOrNumber — licenseKey, forwardingNumber, or assistantId
 * @param {number} durationSeconds
 * @returns {object} { callMinutes, previousMinutes, newTotalMinutes, previousBalance, newBalance, isPaused, subscriber }
 */
async function incrementMinutesUsed(keyOrNumber, durationSeconds) {
  const db = getDb();
  // True whole-number metering: round to nearest whole minute (e.g. 4.3 -> 4, 15.6 -> 16)
  const callMinutes = Math.round(durationSeconds / 60);
  if (!db) {
    return {
      success: true,
      callMinutes,
  ...
  let docRef = db.collection('voice_pro_bindings').doc(keyOrNumber);
  let docSnap = await docRef.get();

  // 2. Fall back to querying by forwardingNumber, vapiPhoneNumber, or vapiAssistantId
  if (!docSnap.exists) {
    const snapshot = await db.collection('voice_pro_bindings')
      .where('forwardingNumber', '==', keyOrNumber)
      .limit(1)
    ...
    if (!snapshot.empty) {
      ...
      const asstSnap = await db.collection('voice_pro_bindings')
        .where('vapiAssistantId', '==', keyOrNumber)
        .limit(1)
        .get();
      if (!asstSnap.empty) {
        docRef = asstSnap.docs[0].ref;
        docSnap = asstSnap.docs[0];
      } else {
        docRef = null;
      }
    }
  ...
  if (docRef && docSnap.exists) {
    const result = await db.runTransaction(async (transaction) => {
      ...
      const currentBalance = typeof data.voiceMinutesBalance === 'number' ? data.voiceMinutesBalance : (data.quotaMinutes || 0);
      const newTotalMinutes = previousMinutes + callMinutes;
      const newBalance = Math.max(0, Math.round(currentBalance - callMinutes));
      const isPaused = newBalance <= 0;

      transaction.update(docRef, {
        minutesUsed: newTotalMinutes,
        voiceMinutesBalance: newBalance,
        isVoicePaused: isPaused,
        voiceActive: !isPaused,
        aiSmsActive: !isPaused,
        lastCallAt: safeFieldValue.serverTimestamp(),
        lastCallDurationSeconds: durationSeconds,
        lastCallMinutesBilled: callMinutes
      });
      ...
      return {
        ...
        newBalance,
        isPaused,
        subscriber: { ...data, id: doc.id, voiceMinutesBalance: newBalance, isVoicePaused: isPaused }
      };
    });

    if (result.subscriber && result.subscriber.licenseKey) {
      try {
        await db.collection('master_licenses').doc(result.subscriber.licenseKey).set({
          voiceMinutesBalance: result.newBalance,
          isVoicePaused: result.isPaused,
          voiceActive: !result.isPaused,
          aiSmsActive: !result.isPaused,
          lastCallAt: safeFieldValue.serverTimestamp()
        }, { merge: true });
      } catch (mErr) {}
    }

    console.log(`[Firestore] Metered: ${callMinutes} mins for ${keyOrNumber} (Total Used: ${result.newTotalMinutes}m, Balance: ${result.newBalance}m, Paused: ${result.isPaused})`);
    return result;
  } else {
    ...
    console.log(`[Firestore] Incremented global minutesUsed by ${callMinutes} (no specific binding for ${keyOrNumber})`);
    return {
      success: true,
      callMinutes,
      newBalance: 0,
