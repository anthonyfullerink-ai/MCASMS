...
function getDb() {
  ...
  if (admin.apps.length === 0) {
    ...
    if (serviceAccountRaw) {
      ...
      // Fallback: Application Default Credentials (works with gcloud auth / Cloud Run / App Engine)
      console.warn('[Firestore] FIREBASE_SERVICE_ACCOUNT_KEY not set — using Application Default Credentials');
      credential = admin.credential.applicationDefault();
    }
 ...
 * @returns {object|null}
 */
async function getVoiceBinding(licenseKey) {
  const db = getDb();
  const doc = await db.collection('voice_pro_bindings').doc(licenseKey).get();
...
async function deactivateVoiceSubscriber(stripeSubscriptionId) {
  ...
  const data = doc.data();
  console.log(`[Firestore] Voice binding deactivated for key: ${data.licenseKey} (sub: ${stripeSubscriptionId})`);
...
async function incrementMinutesUsed(forwardingNumber, durationSeconds) {
  ...
  // Try to find subscriber binding by forwarding number
  const snapshot = await db.collection('voice_pro_bindings')
  ...
  if (!snapshot.empty) {
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      const quotaMinutes = data.quotaMinutes || 200;
      ...
      transaction.update(docRef, {
        ...
        overageMinutesBilled: admin.firestore.FieldValue.increment(newOverageMinutes),
    ...
    await settingsRef.set({
      minutesUsed: admin.firestore.FieldValue.increment(callMinutes),
      lastCallAt: admin.firestore.FieldValue.serverTimestamp()
    }, { merge: true });
